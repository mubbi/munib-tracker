import type { BattlesLessonCard } from "../types/battles";

/** Bump when lesson cards change. */
export const BATTLES_LESSON_CARDS_VERSION = 2;

export const BATTLES_LESSON_CARDS: BattlesLessonCard[] = [
  {
    id: "badr-trust",
    battleId: "badr",
    battleTitle: "Battle of Badr",
    lesson: "Trust Allah while preparing fully.",
    detail:
      "Outnumbered roughly three to one, the Muslims still chose their ground at the wells, arranged their ranks, and made dua — the Prophet ﷺ imploring Allah through the night until his cloak fell. Effort and reliance together, and the victory was credited to Allah.",
  },
  {
    id: "uhud-obedience",
    battleId: "uhud",
    battleTitle: "Battle of Uhud",
    lesson: "Obedience to command protects the community.",
    detail:
      "Archers who left their post seeking spoils opened a flank that nearly destroyed the army. The Qur'an records this as a lesson for every generation about discipline.",
  },
  {
    id: "trench-planning",
    battleId: "trench",
    battleTitle: "Battle of the Trench",
    lesson: "Planning and consultation multiply strength.",
    detail:
      "Salman's trench idea, combined with the Prophet's shura and the believers' labour, neutralised a coalition far larger than the defenders.",
  },
  {
    id: "hudaybiyyah-patience",
    battleId: "hudaybiyyah",
    battleTitle: "Treaty of Hudaybiyyah",
    lesson: "Patience can be the truest victory.",
    detail:
      "The companions felt the truce as a humiliation, and Umar questioned it openly; Surah al-Fath answered by calling it a manifest victory. The ten-year peace opened the roads, Islam spread faster than ever, and within two years the way to Makkah lay open.",
  },
  {
    id: "qurayzah-covenant",
    battleId: "banu-qurayzah",
    battleTitle: "Banu Qurayzah",
    lesson: "Breaking a covenant of protection is among the gravest betrayals.",
    detail:
      "Banu Qurayzah tore up the covenant of Madinah at the very moment the Confederates besieged the city. Even so, the Prophet ﷺ let them be judged by an arbiter they themselves chose — justice through an agreed judge, not unchecked vengeance.",
  },
  {
    id: "makkah-mercy",
    battleId: "conquest-makkah",
    battleTitle: "Conquest of Makkah",
    lesson: "Mercy after victory elevates a conqueror.",
    detail:
      "With the city that had tortured and expelled him now at his mercy, the Prophet ﷺ echoed the words of Prophet Yusuf: 'No blame upon you today — go, you are free.' A general amnesty replaced revenge, and hearts were won that no punishment could have won.",
  },
  {
    id: "hunayn-humility",
    battleId: "hunayn",
    battleTitle: "Battle of Hunayn",
    lesson: "Numbers and recent success do not guarantee victory.",
    detail:
      "Pride in the size of the army after Makkah contributed to initial panic. The believers regrouped only when they turned back to the Prophet ﷺ and to Allah.",
  },
  {
    id: "tabuk-sacrifice",
    battleId: "tabuk",
    battleTitle: "Expedition of Tabuk",
    lesson: "Sacrifice in hardship exposes true faith.",
    detail:
      "The march occurred in scorching heat against a formidable enemy. Those who gave their wealth and those who marched despite poverty were praised alike.",
  },
  {
    id: "mutah-leadership",
    battleId: "mutah",
    battleTitle: "Battle of Mu'tah",
    lesson: "Leadership succession must be planned.",
    detail:
      "The Prophet ﷺ named three commanders in order of succession. When all three fell, Khalid reorganised the retreat — saving the army was itself a victory.",
  },
];
