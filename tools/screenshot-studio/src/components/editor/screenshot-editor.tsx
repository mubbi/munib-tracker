...
export function ScreenshotEditor() {
  ...
  async function exportAll() {
    ...
    // Prefer writing each PNG to exports/ via the dev API (keeps memory flat
    // for big locale × size matrices). Falls back to the zip download when the
    // endpoint is unavailable (e.g. static builds).
    async function writeToDisk(relPath: string, dataUrl: string): Promise<boolean> {
      try {
        const res = await fetch("/api/export", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ path: relPath, dataUrl }),
        });
        return res.ok;
      } catch {
        return false;
      ...
      for (const size of sizes) {
        for (let i = 0; i < currentSlides.length; i++) {
          ...
          try {
            ...
            if (await writeToDisk(path, dataUrl)) {
              savedToDisk += 1;
            } else {
    ...
    if (failed === 0) {
      toast.success(
        savedToDisk > 0
          ? `Exported ${okCount} PNGs to exports/ (${summary})`
          : `Exported ${okCount} PNGs (${summary})`,
      );
    } else if (okCount === 0) {
