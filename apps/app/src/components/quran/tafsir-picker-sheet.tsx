...
import {
  getAllTafsirEditions,
  groupTafsirsByLanguage,
  languageForTafsirId,
  type TafsirLanguageGroup,
} from "@/lib/quran-tafsir-options";
import { useChevronBack, useChevronForward } from "@/lib/rtl";

type TafsirPickerSheetProps = {
  visible: boolean;
  title: string;
  selectedId: string;
...
const ALL_TAFSIRS = getAllTafsirEditions();

export function TafsirPickerSheet({
  visible,
  title,
  selectedId,
  onSelect,
  ...
  const languageGroups = useMemo(
    () => groupTafsirsByLanguage(ALL_TAFSIRS, preferredLanguages),
    [preferredLanguages],
  );
  ...
  useEffect(() => {
    ...
    if (selectedId) {
      const language = languageForTafsirId(selectedId, ALL_TAFSIRS);
      if (language) {
  ...
  const filteredEditions = useMemo(() => {
    ...
    return activeGroup.editions.filter((edition) => {
      const haystack = `${edition.name} ${edition.author}`.toLowerCase();
      return haystack.includes(q);
    });
  ...
  const headerTitle =
    step === "languages"
      ? title
      : t("quran.selectTafsirAuthor", { language: activeGroup?.name ?? "" });

  const searchPlaceholder =
    step === "languages" ? t("quran.searchTafsirLanguages") : t("quran.searchTafsirAuthors");

  const renderLanguageRow = ({ item }: { item: TafsirLanguageGroup }) => {
    ...
    const secondary = item.locale
      ...
      : item.english;
    const countLabel = t("quran.tafsirCount", { count: item.editions.length });
  ...
  const renderEditionRow = ({ item }: { item: (typeof ALL_TAFSIRS)[number] }) => {
        ...
        <FlatList
          ...
          ListHeaderComponent={
            allowNone ? (
              ...
              >
                <View style={styles.rowText}>
                  <ThemedText type="small">{t("quran.tafsirNone")}</ThemedText>
                </View>
                {selectedId === "" ? (
                  <SymbolView
                    name={{ ios: "checkmark", android: "check", web: "check" }}
