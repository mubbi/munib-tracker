...
function renderDuas() {
  ...
  const body = DUA_ITEMS.map((item) => serializeObject(item, order)).join("\n");
...
function renderDuroods() {
  ...
  const body = DUROOD_ITEMS.map((item) => serializeObject(item, order)).join("\n");
...
export async function buildAdhkar() {
  ...
  await writeFileStable(zikrPath, renderZikr());
  await writeFileStable(duasPath, renderDuas());
  await writeFileStable(duroodsPath, renderDuroods());
  console.log(
    `  zikr.ts → ${ZIKR_ITEMS.length}, duas.ts → ${DUA_ITEMS.length}, duroods.ts → ${DUROOD_ITEMS.length}`,
  );
  ...
  return [
    await datasetEntry({
      ...
      license: "Public domain (classical texts of Qur'an & Hadith)",
      attribution: "Adhkar & duas curated from Hisnul Muslim (Sa'id bin Ali al-Qahtani).",
