...
function renderZikr(audioById) {
  ...
  const body = withAudio(ZIKR_ITEMS, audioById)
...
export const ZIKR_CONTENT_VERSION = 3;
...
function renderDuas(audioById) {
  ...
  const body = withAudio(DUA_ITEMS, audioById)
    .map((item) => serializeObject(item, order))
    .join("\n");
...
function renderDuroods(audioById) {
  const order = [
    "id",
    "title",
    "arabic",
  ...
  const body = withAudio(DUROOD_ITEMS, audioById)
    .map((item) => serializeObject(item, order))
    .join("\n");
  return `import type { DurudItem } from "../types/index";

export const DUROOD_CONTENT_VERSION = 3;
...
function assertContent() {
  ...
  for (const cat of cats) {
    const count = ZIKR_ITEMS.filter((i) => i.categoryId === cat).length;
...
export async function buildAdhkar() {
  ...
  const audioById = {};
  let matched = 0;
  for (const item of [...ZIKR_ITEMS, ...DUA_ITEMS, ...DUROOD_ITEMS]) {
  ...
  await writeFileStable(zikrPath, renderZikr(audioById));
  await writeFileStable(duasPath, renderDuas(audioById));
  await writeFileStable(duroodsPath, renderDuroods(audioById));
  console.log(
    `  zikr.ts → ${ZIKR_ITEMS.length}, duas.ts → ${DUA_ITEMS.length}, duroods.ts → ${DUROOD_ITEMS.length} (${matched} with audio)`,
  );
  ...
  return [
    await datasetEntry({
      ...
      attribution: `Adhkar & duas curated from Hisnul Muslim. Audio matched to ${HISN_AUDIO_REPO}.`,
