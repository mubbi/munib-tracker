...
@Injectable()
export class SurfacePushQStashService {
  ...
  async schedule(jobId: string, executeAt: Date): Promise<string | null> {
    ...
    const result = await this.client.publishJSON({
      ...
      retries: 5,
      timeout: 20,
      deduplicationId: `surface-push-${jobId}`,
      label: "surface-push",
    });
  ...
  async cancel(messageIds: string[]): Promise<void> {
    ...
    try {
      ...
      // Delivered/in-flight messages cannot be cancelled.
    }
  }
  ...
  private callbackUrl(): string | null {
    ...
    return origin ? `${origin}/api/v1/surface-push/internal/deliver` : null;
  }
}
