...
- Set production Postgres (`DATABASE_URL` Session pooler URI, or discrete `DATABASE_*`), `JWT_SECRET` (≥32 chars), and `CORS_ORIGINS` on the API project. Build-time migrations need these vars available to **Production** builds (otherwise TypeORM falls back to `localhost:5432` and the deploy fails).
- **Supabase Postgres:** use the **Session** pooler connection (port `5432`). Prefer the same `DATABASE_URL` as admin/marketing on the API project; discrete `DATABASE_HOST` / `DATABASE_PORT` / `DATABASE_USER` / `DATABASE_PASSWORD` / `DATABASE_NAME` still work when `DATABASE_URL` is unset (see `apps/api/.env.example`). TLS is auto-enabled when the host contains `supabase`. Migration `HardenSupabaseRls` enables forced RLS on all app tables and revokes Data API grants from `anon` / `authenticated` / `service_role` — Nest and admin keep working because they connect as the privileged DB role (`BYPASSRLS`). Do **not** put the Supabase publishable (`anon`) key in any Munib app; optionally turn off unused Data API exposure in the Supabase dashboard.
