...
> **Archived copy (2026-07-09).** Active docs: [`../FEATURES.md`](../FEATURES.md) · [`../BACKLOG.md`](../BACKLOG.md) · [`../README.md`](../README.md)
>
...
**Every software-achievable item is shipped.** All P0 (8), all P1 (including NF-1.11 mushaf), and all P2 that don't need an external dataset are `done`, greenfield, en/ar/ur, with gates green (shared 100 + app 315 tests, full-repo Biome clean, app/shared/api `check-types` clean). The only open items are blocked on bundled content datasets — see ⛔ below.

Legend: ✅ done · ⛔ blocked (needs an external dataset — deliberately not fabricated).
...
| NF-1.11 | Mushaf page view | ✅ | `build-mushaf-layout.mjs`, `/quran/pages`, `/quran/page/[page]`, ayah/page/mushaf layouts + QCF font cache |
...
| NF-2.9 | Narrator chain display | ⛔ | `narrator` attribution shown; full isnad chain needs structured dataset on `HadithItem` |
...
| NF-2.17 | App lock (PIN / biometrics) | ✅ | `features/pin-lock` + `expo-local-authentication`; `/settings/app-lock`; `PinLockGate` |
...
| NF-1.11 | **Mushaf page view** ✅ | app | `build-mushaf-layout.mjs`, `/quran/pages`, `/quran/page/[page]`, `qcf-font-cache.ts` | 604-page Madani layout; ayah/page/mushaf modes; page jump |
...
| 2026-07-09 | **Doc sync — NF-1.11 + NF-2.17 marked done** (were shipped in `f35f2be` but still listed ⛔): **NF-1.11** mushaf — `build-mushaf-layout.mjs`, 604 `layout/madinah-15` pages, `/quran/pages` + `/quran/page/[page]`, ayah/page/mushaf reader layouts, on-demand QCF fonts (`qcf-font-cache.ts`). **NF-2.17** app lock — `features/pin-lock`, `expo-local-authentication`, `/settings/app-lock`, root `PinLockGate`. **Still open (data only):** NF-2.7 word-by-word, NF-2.8 hadith sharh, NF-2.9 full isnad chain (`narrator` string already shown). |
