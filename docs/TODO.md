...
**Product gaps (unchanged):** client auth, sync engine, notifications, audio — Phases 6, 8–12.

---

### Implementation progress — 2026-07-03

Phases **1–5 and 7 are implemented, typechecked, and tested** (44 tests: 27 Vitest in `@munib-tracker/shared`, 17 Jest in `apps/app`; repo-wide `check-types` and Biome clean).

**Key decisions / deviations from the original PRD (all deliberate):**

- **Persistence uses AsyncStorage, not `expo-sqlite`** (`apps/app/src/db/`). Standing constraint is no new native deps forcing a dev-client rebuild, and web must keep working. Repositories (`Prayer`/`Zikr`/`Qaza`/`Preferences`) sit behind a `KeyedCollection` abstraction with versioned migrations — the storage engine can be swapped later without touching callers.
- **State layer is a zero-dep `useSyncExternalStore` store** (`apps/app/src/stores/create-store.ts`), not zustand — same ergonomics, no dependency. `tracker-store` is the reactive hub; `preferences-store` mirrors `UserPreferences`.
- **Navigation restructured** into a root `Stack` + `(tabs)` group so detail screens push over the tab bar (`apps/app/src/app/(tabs)/`). `ScreenLayout`/`AppHeader` gained an `onBack`. The generated `.expo/types/router.d.ts` was stale; it's regenerated by Metro on next run.
- **Charts and the tasbeeh ring are dependency-free** (View/conic-gradient), honoring the same native-dep constraint.

**What's done per phase:** P1 data layer + streak/summary utils · P2 prayer statuses + notes + dashboard · P3 zikr library/favorites/tasbeeh (content seed in `packages/shared/src/content`) · P4 counters/calculator/planner/roza · P5 calendar/day-detail/statistics · P7 settings index + appearance/notifications/bedtime/fonts/language/about.

**Deferred (need native modules or external credentials — out of the no-rebuild scope):** P6 audio player + libraries (`expo-audio`), P8 client auth + sync (`expo-auth-session`/`expo-secure-store` + OAuth creds), P9 notifications (`expo-notifications`), P10 onboarding/achievements, P11 profile, P12 qibla (`expo-sensors`/`expo-location`). Dua and 99-Names routes exist as placeholders; full i18n string extraction (P7.5) and the custom-hex accent (P7.6) are partial.
